...
namespace Testing
{
    public partial class Form1 : Form
    {
        ...
        //Функция подсчета баллов для вопросов с несколькими вариантами ответа
        double graduate(ArrayList lst1, ArrayList lst2)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                //bw2.Write("Был выбран вариант ответа: " + lst1[i].ToString() + "\r\n");
                //bw2.Write("Верный вариант ответа: " + lst2[i].ToString() + "\r\n");
                //strmwrt.WriteLine("Был выбран вариант ответа: " + lst1[i].ToString() + "\r\n");
                //strmwrt.WriteLine("Верный вариант ответа: " + lst2[i].ToString() + "\r\n");
            ...
            {
                grade = 3.0;
                bw2.Write("Все ответы отмечены верно, балл за вопрос 3!\r\n");
                strmwrt.WriteLine("Все ответы отмечены верно, балл за вопрос 3!\r\n");
            }
            else if (tmp2 > tmp1)
            {
                grade = 0;
                bw2.Write("Было отмечено слишком много пунктов, полученный балл за вопрос 0\r\n");
                strmwrt.WriteLine("Было отмечено слишком много пунктов, полученный балл за вопрос 0\r\n");
            }
            ...
            if ((tmp1 > tmp2) || (!array1.SequenceEqual(array2) && tmp1 == tmp2))
            {
                ...
                bw2.Write("Из "+tmp1.ToString()+" верных вариантов отвечено верно "+tmp3.ToString() +" полученный балл: " + grade.ToString() + "\r\n");
                strmwrt.WriteLine("Из " + tmp1.ToString() + " верных вариантов отвечено верно " + tmp3.ToString() + " полученный балл: " + grade.ToString() + "\r\n");
            }
        ...
        //Функция, навешанная на кнопку вход. Аутентификация и авторизация
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            //{
                ...
                string cmdtxt = "SELECT * FROM [Авторизация] WHERE [Логин] =" + "\"" + Login.Text + "\"" + " AND [Пароль] =" + "\"" + Password.Text + "\"";

                OleDbDataAdapter dta3 = new OleDbDataAdapter(cmdtxt, connection_string);


                //создаем дата сет, куда запишем данные из запроса
                ...
                if (dt.Tables[0].Rows.Count == 1)
                {
                    ...
                    //выдаем права в соответствии с записью в базе данных:
                    //Добавление записей
                    this.администрированиеToolStripMenuItem.Visible = Convert.ToBoolean(dt.Tables[0].Rows[0][4].ToString());
        ...
        //Выгрузка базы всех вопросов
        public void выгрузитьБазуВопросовToolStripMenuItem_Click(object sender, EventArgs e)
        {
            

            try
            {
                string cmd = "SELECT * FROM [Вопросы]"; //Запрос на выборку всех вопросов перого типа
                ...
                string cmd1 = "SELECT * FROM [Вопрос1]";// То же самое для второго типа
                ...
                string cmd2 = "SELECT * FROM [Вопросы2]";// То же для третьего типа
        ...
        //Добавление вопросов
        private void отдельныйВопросToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
        ...
        private void добавитьПользователяToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string qry1 = "SELECT COUNT(*) FROM [Авторизация]";
        ...
        private void удалитьПользователяToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                string cmd = "SELECT Код,Логин,Пароль,ФИО,Должность FROM [Авторизация]";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //try
            //{
                string qry_for_id_s = "SELECT * From ID_Сессии";
                ...
                string qry_for_upd_id_s = "UPDATE [ID_Сессии] SET ID = " + id_s.ToString();
                ...
                string qry2 = "Select * From Вопросы";
                string qry3 = "Select * From Вопрос1";
                string qry4 = "Select * From Вопросы2";
                ...
                int count_of_questions_type1 = dsforcount.Tables[0].Rows.Count; //Подсчет количества вопросов в бд для фуникции рандомной выборки вопросов
                int count_of_questions_type2 = dsforcount1.Tables[0].Rows.Count;
                int count_of_questions_type3 = dsforcount2.Tables[0].Rows.Count;

                arrlst = rndarray(25, count_of_questions_type1); //Формируем рандомно номера вопросов каждого типа
                arrlst2 = rndarray(10, count_of_questions_type2);
                arrlst3 = rndarray(5, count_of_questions_type3);

                int grade = 0;

                 string qry_for_name1 = "SELECT ФИО FROM Авторизация WHERE Логин=" + "\"" + Login.Text + "\"";
                ...
                int j = 0;
                //25 вопросов первого типа
                while (j != 25 && break_flag == false)
                {
                    

                    string qry = "Select Вопрос,Вариант1,Вариант2,Вариант3,Вариант4,Верный,Балл FROM Вопросы WHERE Код=" + arrlst[j].ToString();
                    ...
                    qst1.Text = "Вопрос №  " + (i + 1).ToString(); // номер вопроса
                    qst1.textBox1.Text = ds.Tables[0].Rows[0][0].ToString();// Текст вопроса
                    qst1.var1.Text = ds.Tables[0].Rows[0][1].ToString();//Первый вариант ответа
                    qst1.var1.SelectionStart = 0;
                    bw2.Write(DateTime.Now.ToString() + "\r\n");//дата в логи
                    bw2.Write(ds.Tables[0].Rows[0][0].ToString() + "\r\n");//Текст вопроса в логи

                    strmwrt.WriteLine(ds.Tables[0].Rows[0][0].ToString()+"\r\n");
                    qst1.var2.Text = ds.Tables[0].Rows[0][2].ToString();//Второй вариант ответа
                    ...
                    bw2.Write("Выбранный ответ:" + qst1.textBox2.Text + "\r\n");//Логи
                    strmwrt.WriteLine("Выбранный ответ:" + qst1.textBox2.Text + "\r\n");
                    bw2.Write("Верный ответ:" + ds.Tables[0].Rows[0][5].ToString() + "\r\n");
                    strmwrt.WriteLine("Верный ответ:" + ds.Tables[0].Rows[0][5].ToString() + "\r\n");

                    OleDbDataAdapter dta_for_true_answer_part_1 = new OleDbDataAdapter("Select * from [Вопросы] where Вопрос ="+"\""+qst1.textBox1.Text.ToString()+"\"",connection_string);
                    ...
                    if (qst1.textBox2.Text == ds.Tables[0].Rows[0][5].ToString())//Если ответ верный 2 балла в плюс
                    {
                        grade = grade + 2;
                    ...
                    else if (qst1.textBox2.Text == "5")
                    {
                        ...
                        break_flag = true;
                        bw2.Write(DateTime.Now.ToString() + "\r\n");
                        bw2.Write("Тест был завершен пользователем на " + i.ToString() + " вопросе\r\n");
                        strmwrt.WriteLine("Тест был завершен пользователем на " + i.ToString() + " вопросе\r\n");
                    }
                    else// ничего , если не верный, так же запись об этом  в логи
                    {
                    ...
                    string qryansw = "INSERT INTO [Ответы1_3]  (Логин,ID,Пользователь,Вопрос,Ответ,Верный,Балл,Дата)  Values (" + "\"" + Login.Text + "\"" + "," + "\"" + id_s.ToString() + "\"" + "," + "\"" + user1.ToString() + "\"" + "," + "\"" + qst1.textBox1.Text.ToString() + "\"" + "," + "\"" + current_answer_part_1 + "\"" + "," + "\"" + true_answer_part_1 + "\"" + "," + "\"" + current_grade + "\"" + "," + "\"" + DateTime.Now.ToShortDateString() + "\"" + ")";
                ...
                while (k != 10 && break_flag == false)
                {
                    ...
                    qst2.Text = "Вопрос №  " + (i + 1).ToString(); // Порядковый номер


                    string qry = "Select Вопрос,Ответ1,Ответ2,Ответ3,Ответ4,Ответ5,Ответ6,Верный1,Верный2,Верный3,Верный4,Верный5,Верный6,Балл FROM Вопрос1 WHERE Код=" + arrlst2[k].ToString();// Выборка вопрсов
                    ...
                    qst2.textBox1.Text = ds.Tables[0].Rows[0][0].ToString();//Вопрос
                    strmwrt.WriteLine("Вопрос: " + ds.Tables[0].Rows[0][0].ToString() + "\r\n");
                    qst2.textBox2.Text = ds.Tables[0].Rows[0][1].ToString();//Вариант 1
                    ...
                    lst1.Add(qst2.checkBox1.Checked.ToString());//Тут в массивы записываются выбранные варианты ответов
                    ...
                    bw2.Write(DateTime.Now.ToString() + "\r\n");//Время в логи
                    grade2 = grade2 + graduate(lst1, lst2);//Вызов фунции подсчета баллов


                    qst2.checkBox1.Checked = false;//Сброс чекбоксов на false для нового вопроса
                ...
                {
                    qst3.Text = "Вопрос №  " + (i + 1).ToString();// Тут Аналогично первому типу вопросов




                    string qry = "Select Вопрос,Ответ,Балл FROM Вопросы2 WHERE Код=" + arrlst3[l].ToString();
                    ...
                    bw2.Write("\r\nВопрос: " + qst3.textBox1.Text + "\r\n");
                    strmwrt.WriteLine("\r\nВопрос: " + qst3.textBox1.Text + "\r\n");
                    bw2.Write("\r\nВерный ответ: " + ds.Tables[0].Rows[0][1].ToString() + "\r\n");
                    strmwrt.Write("\r\nВерный ответ: " + ds.Tables[0].Rows[0][1].ToString() + "\r\n");
                    ...
                    {
                        bw2.Write("Ответ верный\r\n");
                        strmwrt.Write("Ответ верный\r\n");
                        grade3 = grade3 + 4;
                        current_grade = "4";
                    }
                    else
                    {
                        bw2.Write("Ответ не верный\r\n");
                        strmwrt.Write("Ответ не верный\r\n");
                    }
                    ...
                    if (qst3.textBox3.Text == "5")
                    {
                        ...
                        bw2.Write(DateTime.Now.ToString() + "\r\n");
                        bw2.Write("Тест был завершен пользователем на " + i.ToString() + " вопросе\r\n");
                        strmwrt.Write("Тест был завершен пользователем на " + i.ToString() + " вопросе\r\n");
                    }

                    string qryansw = "INSERT INTO [Ответы3]  (Логин,ID,Пользователь,Вопрос,Ответ,Верный,Балл,Дата)  Values (" + "\"" + Login.Text + "\"" + "," + "\"" + id_s.ToString() + "\"" + "," + "\"" + user1.ToString() + "\"" + "," + "\"" + qst3.textBox1.Text.ToString() + "\"" + "," + "\"" + qst3.textBox2.Text.ToString() + "\"" + "," + "\"" + ds.Tables[0].Rows[0][1].ToString() + "\"" + "," + "\"" + current_grade + "\"" + "," + "\"" + DateTime.Now.ToShortDateString() + "\"" + ")";
                    ...
                    MessageBox.Show("Итоговый балл: " + (Convert.ToDouble(grade) + grade2 + Convert.ToDouble(grade3)).ToString() + " / 100");//Вывод итогового балла
                    bw2.Write(DateTime.Now.ToString() + "\r\n");
                    bw2.Write("Итоговый балл: " + (Convert.ToDouble(grade) + grade2 + Convert.ToDouble(grade3)).ToString() + " / 100 \r\n");//Запись в логи
                    strmwrt.Write("Итоговый балл: " + (Convert.ToDouble(grade) + grade2 + Convert.ToDouble(grade3)).ToString() + " / 100 \r\n");
                    bw2.Close();
                    strmwrt.Close();
                    ...
                    string qry_for_name = "SELECT ФИО FROM Авторизация WHERE Логин=" + "\"" + Login.Text + "\"";
                    ...
                    string qry_r = "INSERT INTO [Результаты_Теста] (Пользователь,ID,Дата,Балл,Логин) Values (" + "\"" + user + "\"" + ","+"\""+ id_s.ToString() + "\"" +","+ "\"" + DateTime.Now.ToShortDateString() + "\"" + "," + "\"" + grd + "\""+"," +"\""+Login.Text+"\""+ ")";
